...
<body class="mn-ht-100v d-flex flex-column">
  ...
  <div class="content pd-0 mg-l-0" style="position: relative">
            ...
            <div id="content-canvas" class="row row-xs bg-0 mg-0 mg-lg-x-10 pd-0 flex-1"
              ...
              <div class="min-height-fix write-content-here wd-100p">
                ...
                <div class="tab-content">
                  <div class="tab-pane fade" id="first" style="margin-top: -4px;">
                  ...
                  <div class="wd-100p tab-pane fade card static active in show bd-0 pd-x-20" id="second"
    ...
    <div class="modal fade" id="cardEditModal" tabindex="-1" role="dialog"
      ...
      <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body bd-0 pd-y-0 pd-x-20">
            <div class="d-flex flex-column wd-100p">
              <div class="d-flex wd-100p mg-b-10 rounded-its-10 pd-0 mg-0" style="pointer-events: none !important;">
                <div class="btn btn-custom-select pd-x-15 pd-y-7 rounded-its-6 ctgr mg-r-6">Seminar</div>
                <div class="btn btn-custom-select pd-x-15 pd-y-7 rounded-its-6 ctgr mg-r-6">Kelas</div>
                <div class="btn btn-custom-select pd-x-15 pd-y-7 rounded-its-6 ctgr mg-r-6">Rapat</div>
                <div class="btn btn-custom-select pd-x-15 pd-y-7 rounded-its-6 ctgr mg-r-6">Luar</div>
              ...
              <div class="wd-100p mg-b-10 rounded-its-6 pd-0 mg-0 pd-x-4">
                ...
                <div class="pd-x-4 pd-y-10 input-bg-color inside-border pos-relative"
                  ...
                  <input type="text" value="" placeholder="Nama Kegiatan" id="namakegiatandetail"
                    class="wd-100p  tx-14 mg-l-4 tx-color-01 tx-medium"
                    style="border-radius: 10px; background-color: none !important; border: none !important;" readonly></input>
              ...
              <div class="row row-xs mg-b-0 pd-x-4">
                ...
                <div class="col-4">
                  <div class="row row-xs wd-100p mg-b-10 rounded-its-6 pd-0 mg-0">
                    ...
                    <div class="col-12 pd-x-4 pd-y-10 rounded-its-6 input-bg-color inside-border">
                      <input type="text" id="durasidetail" name="durasidetail" placeholder="0 Jam 0 Menit"
                        class="mn-wd-100p tx-14 mg-l-4 tx-color-01 tx-medium"
                        style="border-radius: 10px; background-color: none !important; border: none !important;"
                        readonly></input>
                      <button
              ...
              <div class="tx-color-01 tx-14 pd-x-5 d-inline-block text-break"" style="margin-top: -1px; ">Kegiatan akan dilaksanakan pada <span
                  id="daydetail" class="d-inline-block text-break"></span> <span id="pukuldetail"> pukul <span id="stdetail"></span> sampai <span
                    id="eddetail"></span></span><span id="selamadetail"></span>
              </div>
            </div>
            <div class="modal-footer bd-0 pd-x-4">
              <button type="button" class="btn btn-primary tx-medium rounded-its-6" data-dismiss="modal" id="detaildone">Kembali</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <!-- Generate weekly days and date dynamically  -->
    <!-- OnClick date item to show new reserve modal and detail modal -->
    <script>
      $(function () {
        'use strict'
        ...
        function showCardDetail(target, currTarget, thisClick) {
          let targetClass = target.className;
          let currTargetClass = currTarget.className;

          $('.btn-custom-select').removeClass("choosen");
          $('.btn-custom-select:contains("'+ $(target).closest("div.schedule-card-con").attr("data-passed").split("**").at(-4) +'")').addClass("choosen");
          $("#waktuselesaidetail").val($(target).closest("div.schedule-card-con").attr("data-passed").split("**").at(-1) + " WIB");
          $("#waktumulaidetail").val($(target).closest("div.schedule-card-con").attr("data-passed").split("**").at(-2)  + " WIB");
          $("#namakegiatandetail").val($(target).closest("div.schedule-card-con").attr("data-passed").split("**").at(-3));

          let tmStart = new Date(new Date().toDateString("yyyy-MM-dd") + " " + document.getElementById('waktumulaidetail').value.split(" ").at(-2));
          let tmEnd = new Date(new Date().toDateString("yyyy-MM-dd") + " " + document.getElementById('waktuselesaidetail').value.split(" ").at(-2));
          let diff = ((new Date(tmEnd) - new Date(tmStart)) / 60 / 1000);
          let hr = Math.floor(diff / 60);
          let mn = diff % 60;

          $('#durasidetail').val(hr + " jam " + mn + " menit");
          $('#daydetail').html($(thisClick).attr("data-hari"))
          $('#stdetail').text(document.getElementById('waktumulaidetail').value.split(" ").at(-2));
          $('#eddetail').text(document.getElementById('waktuselesaidetail').value.split(" ").at(-2));
          $('#selamadetail').text(" selama " + hr + " jam " + mn + " menit");
        ...
        $('.date-item').click(function (e) {
          ...
          if (e.target === e.currentTarget || e.target.className === 'date-item-line') {
            ...
            showCardDetail(e.target, e.currentTarget, this)
          } else {
            return(e)
          }
        })
